...
///                     Depenedent table data, e.g. for media = 1 Image, Media = 2 Video
///          - Returns - Inserted VideoEvent Id
///         
///     8. Update VideoEvent Table
...
///          - Returns - Inserted VideoEvent Id
///         
///     9. Query Videosegment Table for image/Video files
...
///                     
///     10. Insert VideoSegment Table for image/Video files
...
///          
///     11. Update VideoSegment Table for image/Video files
///         ** converting files to blob (byte[]) and passing
...
namespace TestMediaUpload_UserControl
{
    public partial class MainWindow : Window
    {
        private bool IsSetUp = false;
        private readonly bool PopulateVideoEventFlag = true; // Should be  handled as per need, keeping true for now
        private readonly bool PopulateProjectFlag = true; // Should be  handled as per need, keeping true for now
        ...
        private void cmbMedia_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                btnBrowse.IsEnabled = true;
                //FilterMedia = "All Video Files|*.wav;*.aac;*.wma;*.wmv;*.avi;*.mpg;*.mpeg;*.m1v;*.mp2;*.mp3;*.mpa;*.mpe;*.m3u;*.mp4;*.mov;*.3g2;*.3gp2;*.3gp;*.3gpp;*.m4a;*.cda;*.aif;*.aifc;*.aiff;*.mid;*.midi;*.rmi;*.mkv;*.WAV;*.AAC;*.WMA;*.WMV;*.AVI;*.MPG;*.MPEG;*.M1V;*.MP2;*.MP3;*.MPA;*.MPE;*.M3U;*.MP4;*.MOV;*.3G2;*.3GP2;*.3GP;*.3GPP;*.M4A;*.CDA;*.AIF;*.AIFC;*.AIFF;*.MID;*.MIDI;*.RMI;*.MKV";
        ...
        {
            InsertVideoEventWithAssociatedTable();
        }
        ...
        private void InsertVideoEventWithAssociatedTable()
        {
            try
            {
                var dataTable = new DataTable();
                ...
                var row = dataTable.NewRow();
                row["videoevent_id"] = -1;
                row["fk_videoevent_project"] = 1;
                row["videoevent_track"] = 1;
                row["videoevent_start"] = SelectedMedia?.media_name == "image" ? "00:00:00" : "00:05:10";
                row["videoevent_duration"] = SelectedMedia?.media_name == "image" ? 0: 310;
                row["videoevent_createdate"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                row["videoevent_modifydate"] = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                row["fk_videoevent_media"] = SelectedMedia.media_id;
                
                if (SelectedMedia?.media_name == "image" || SelectedMedia?.media_name == "video")
                {
                    row["fk_videoevent_screen"] = -1; // Not needed for this case
                ...
                MessageBox.Show($"VideoEvent Table populated to Database with id - {insertedIds[0]} along with {SelectedMedia?.media_name}");
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButton.OK, MessageBoxImage.Error);
