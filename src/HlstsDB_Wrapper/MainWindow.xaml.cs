...
///                     
///    7. Query Hlsts Table
///     DataManagerSqlLite.GetHlsts(int projectId = -1, dependentTableFlag)
///         - Comments: Fetch all Hlsts present in the database 
///         - Parameters:   projectId: projectId is -1 then fetch all rows else dependent hlsts rows for the project id passed
///                         dependentTableFlag - If true, also fetched dependent/associated tables (streamts).
///         - Returns - List<CBVHlsts>. Fields are as below -
///                     int - hlsts_id
///                     int fk_hlsts_project
///                     int hlsts_version
///                     string hlsts_comments
///                     byte[] hlsts_media480
///                     byte[] hlsts_media720
///                     byte[] hlsts_media1080
///                     byte[] hlsts_media1280
///                     byte[] hlsts_master
///                     byte[] hlsts_encryption
///                     DateTime hlsts_createdate
///                     DateTime hlsts_modifydate
///                     **List<CBVStreamts> streamts_data - associated table data
///                     
///     8. Insert HLSTS Table
///         DataManagerSqlLite.InsertRowsToDesign(dataTable)
///         -Comments - To add data to design table
///         -Parameters - datatable
///             Columns - hlsts_id, fk_hlsts_project, hlsts_version, hlsts_comments, hlsts_media480, hlsts_media720, 
///                         hlsts_media1080, hlsts_media1280, hlsts_master, hlsts_encryption, hlsts_createdate, hlsts_modifydate
///         -Returns - Inserted Design Id
///          
///     9. Update HLSTS Table
///         ** Reading XML files and passing as Text
///         DataManagerSqlLite.UpdateRowsToHlsts(dataTable)
///         -Comments - To Update data to HLSTS table
///         -Parameters - datatable
///             Columns - hlsts_id, fk_hlsts_project, hlsts_version, hlsts_comments, hlsts_media480, hlsts_media720, 
///                     hlsts_media1080, hlsts_media1280, hlsts_master, hlsts_encryption, hlsts_modifydate
///          -Returns - None
///          
...
///                     
///     12. Query STREAMTS Table
///     DataManagerSqlLite.GetStreamts(int hlstsId = -1)
///         - Comments: Fetch all Streamts rows present in the database
///         - Parameterrs: hlstsId - hlstsId is -1 then fetch all rows else dependent streamts rows for the hlsts id passed
///         - Returns - List<CBVStreamts>. Fields are as below -
///                     int - streamts_id
///                     int fk_streamts_hlsts
///                     int fk_streamts_resolution
///                     int streamts_increment
///                     byte[] streamts_stream
///                     
///      13. Insert STREAMTS Table
///         DataManagerSqlLite.InsertRowsToStreamts(dataTable)
///         -Comments - To add data to Streamts table
///         -Parameters - datatable
///             Columns - streamts_id, fk_streamts_hlsts, fk_streamts_resolution, streamts_increment, streamts_stream
///         -Returns - Inserted Design Id
///          
///     14. Update STREAMTS Table
///         DataManagerSqlLite.UpdateRowsToStreamts(dataTable)
///         -Comments - To Update data to STREAMTS table
///         -Parameters - datatable
///             Columns - streamts_id, fk_streamts_hlsts, fk_streamts_resolution, streamts_increment, streamts_stream
///          -Returns - None
///          
///     15. Query RESOLUTION Table
///     DataManagerSqlLite.GetResolution()
///         - Comments: Fetch all RESOLUTION rows present in the database 
///         - Parameters: NA
///         - Returns - List<CBVResolution>. Fields are as below -
///                     int - resolution_id
...
namespace TimelineDB_Wrapper
{
    public partial class MainWindow : Window
    {
        ...
        private void BtnInsertHlstsData_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Read hlsts Bytes into a byte array - Temporary data
                var currentDirectory = Directory.GetCurrentDirectory();
                var path = $"{currentDirectory}\\MediaFiles\\Image1.jpg";
                using (var fileStream = new FileStream(path, FileMode.Open))
                {
                    ...
                    datatable.Columns.Add("hlsts_version", typeof(int));
                    datatable.Columns.Add("hlsts_comments", typeof(string));
                    ...
                    datatable.Columns.Add("hlsts_master", typeof(byte[]));
                    datatable.Columns.Add("hlsts_encryption", typeof(byte[]));
                    datatable.Columns.Add("hlsts_createdate", typeof(string));
                    datatable.Columns.Add("hlsts_modifydate", typeof(string));

                    var row = datatable.NewRow();
                    row["hlsts_id"] = -1;
                    row["fk_hlsts_project"] = ((CBVProject)cmbProject.SelectedItem)?.project_id ?? 0;
                    row["hlsts_version"] = Convert.ToInt32(txtVersion.Text);
                    row["hlsts_comments"] = txtComments.Text;
                    row["hlsts_media480"] = mp3Byte;
                    row["hlsts_media720"] = mp3Byte;
                    row["hlsts_media1080"] = mp3Byte;
                    row["hlsts_media1280"] = mp3Byte;
                    row["hlsts_master"] = mp3Byte;
                    row["hlsts_encryption"] = mp3Byte;
                    ...
                    if (insertedId > -1)
                    {
                        MessageBox.Show("HLSTS Row Added to Database");
                        RefreshOrLoadComboBoxes(EnumEntity.HLSTS);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void BtnInsertStreamtsData_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Read hlsts Bytes into a byte array - Temporary data
                ...
                using (var fileStream = new FileStream(path, FileMode.Open))
                {
                    ...
                    datatable.Columns.Add("streamts_id", typeof(int));
                    datatable.Columns.Add("fk_streamts_hlsts", typeof(int));
                    datatable.Columns.Add("fk_streamts_resolution", typeof(int));
                    datatable.Columns.Add("streamts_increment", typeof(int));
                    datatable.Columns.Add("streamts_stream", typeof(byte[]));
                   

                    var row = datatable.NewRow();
                    row["streamts_id"] = -1;
                    row["fk_streamts_hlsts"] = ((CBVHlsts)cmbHlsts.SelectedItem)?.hlsts_id ?? 0;
                    row["fk_streamts_resolution"] = ((CBVResolution)cmbResolution.SelectedItem)?.resolution_id ?? 0;
                    row["streamts_increment"] = txtIncrement.Text;
                    row["streamts_stream"] = mp3Byte;
                    datatable.Rows.Add(row);

                    var insertedId = DataManagerSqlLite.InsertRowsToStreamts(datatable);
                    if (insertedId > -1)
                    {
                        MessageBox.Show("STREAMTS Row Added to Database");
                        RefreshOrLoadComboBoxes(EnumEntity.HLSTS);
                    }
                }
            }
        ...
        private void RefreshOrLoadComboBoxes(EnumEntity entity = EnumEntity.ALL)
        {
            ...
            {
                var data = DataManagerSqlLite.GetVideoEvents(-1, true);
                FillListBoxVideoEvent(data);
            }

            if (entity == EnumEntity.ALL || entity == EnumEntity.AUDIO)
            {
                var data = DataManagerSqlLite.GetAudio(-1);
                RefreshComboBoxes<CBVAudio>(cmbAudio, data, "audio_id");
            }

            if (entity == EnumEntity.ALL || entity == EnumEntity.VIDEOSEGMENT)
            {
                var data = DataManagerSqlLite.GetVideoSegment(-1);
                RefreshComboBoxes<CBVVideoSegment>(cmbVideoSegment, data, "videosegment_id");
            }


            if (entity == EnumEntity.ALL || entity == EnumEntity.HLSTS)
            {
                var data = DataManagerSqlLite.GetHlsts(-1);
                RefreshComboBoxes<CBVHlsts>(cmbHlsts, data, "hlsts_id");
            }

            if (entity == EnumEntity.ALL || entity == EnumEntity.RESOLUTION)
            {
                var data = DataManagerSqlLite.GetResolution();
                RefreshComboBoxes<CBVResolution>(cmbResolution, data, "resolution_name");
            }
