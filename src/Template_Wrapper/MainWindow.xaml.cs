...
namespace Template_Wrapper
{
    public partial class MainWindow : Window
    {
        ...
        private int currentVideoEventID = -1;
        public MainWindow()
        {
            InitializeComponent();
        }
        ...
        {
            var selectedprojectId = ((CBVProject)cmbProject.SelectedItem)?.project_id ?? 0;
            lblSelectedProject.Content = "Selected  Project Id - " + selectedprojectId;
            var videoEventData = DataManagerSqlLite.GetVideoEvents(selectedprojectId);

            RefreshComboBoxes<CBVVideoEvent>(cmbVideoEvent, videoEventData, "videoevent_id");
        }

        private void CmbVideoEvent_SelectionChanged(object sender, EventArgs e)
        {
            currentVideoEventID = ((CBVVideoEvent)cmbVideoEvent.SelectedItem)?.videoevent_id ?? 0;
            lblSelectedVideoEvent.Content = "Selected Video Event Id - " + currentVideoEventID;
        }
