...
///                     
///     7. Query Design Table
///     DataManagerSqlLiteList<CBVDesign> GetDesign(int videoEventId = -1)
///         - Comments: Fetch all Design rows present in the database 
///         - VideoEventId: VideoEventId is -1 then fetch all rows else dependent design rows for the videoevent id passed
///         - Returns - List<CBVDesign>. Fields are as below -
///                     int - design_id  
///                     int fk_design_videoevent
///                     int fk_design_screen
///                     string design_xml
///                     DateTime design_createdate
///                     DateTime design_modifydate
///          
///     8. Insert Design Table for XML
...
///          
///     9. Update Design Table for XML
///         ** Reading XML files and passing as Text
///         DataManagerSqlLite.UpdateRowsToDesign(dataTable, design_id)
...
///             design_id - id of the row to be updated
///          -Returns - None
///          
...
namespace DesignerDB_Wrapper
{
    public partial class MainWindow : Window
    {
        ...
        {
            var selectedprojectId = ((CBVProject)cmbProjectForDesign.SelectedItem)?.project_id ?? 0;
            var videoEventData = DataManagerSqlLite.GetVideoEvents(selectedprojectId);
            RefreshComboBoxes<CBVVideoEvent>(cmbVideoEventForDesign, videoEventData, "videoevent_id");
        }
        ...
        private void BtnInsertXML_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please add XML Text", "Validation Error");
                    return;
                }
                ...
                rowDesign["fk_design_videoevent"] = ((CBVVideoEvent)cmbVideoEventForDesign.SelectedItem)?.videoevent_id ?? 0;
                rowDesign["fk_design_screen"] = ((CBVScreen)cmbScreenForDesign.SelectedItem)?.screen_id ?? 0;
                ...
                rowDesign["design_xml"] = txtXml.Text;
        ...
        private void BtnGetXML_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    stringText += item.ToString() + Environment.NewLine;
                }
        ...
        private void RefreshOrLoadComboBoxes(EnumEntity entity = EnumEntity.ALL)
        {
            if (entity == EnumEntity.ALL || entity == EnumEntity.PROJECT)
            {
                ...
                RefreshComboBoxes<CBVProject>(cmbProjectForDesign, data, "project_name");
            }

            if (entity == EnumEntity.ALL || entity == EnumEntity.SCREEN)
            {
                var data = DataManagerSqlLite.GetScreens();
                RefreshComboBoxes<CBVScreen>(cmbScreenForDesign, data, "screen_name");
            }
        }
