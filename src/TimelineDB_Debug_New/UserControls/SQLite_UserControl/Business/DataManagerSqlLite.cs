...
namespace Sqllite_Library.Business
{
    public static class DataManagerSqlLite
    {
            ...
            foreach (DataRow rowMain in data.Rows)
            {
                ...
                if (insertedId > 0 && populateDependentTablesFlag)
                {
                    ...
                    else if (mediaId == 4)  //Design
                    {
                        ...
                        rowDesign["fk_design_screen"] = (int)rowMain["fk_videoevent_screen"];
                        rowDesign["design_xml"] = "<note>" +
                                                        "<to>Tove</to>" +
                                                        "<from>Jani</from>" +
        ...
        public static int InsertRowsToHLSTS(DataTable data)
        {
            var videoeventFlag = SqlLiteData.ReferentialKeyPresent("cbv_project", "project_id", (int)data?.Rows[0]["fk_hlsts_project"]);
            if (!videoeventFlag)
                throw new Exception("project_id foreign key constraint not successful ");

            return SqlLiteData.InsertRowsToHLSTS(data);
        }

        public static int InsertRowsToStreamts(DataTable data)
        {
            var hlstsFlag = SqlLiteData.ReferentialKeyPresent("cbv_hlsts", "hlsts_id", (int)data?.Rows[0]["fk_streamts_hlsts"]);
            if (!hlstsFlag)
                throw new Exception("hlsts_id foreign key constraint not successful ");

            var resolutionFlag = SqlLiteData.ReferentialKeyPresent("cbv_resolution", "resolution_id", (int)data?.Rows[0]["fk_streamts_resolution"]);
            if (!resolutionFlag)
                throw new Exception("resolution_id foreign key constraint not successful ");

            return SqlLiteData.InsertRowsToStreamts(data);
        }
        ...
        {
            var insertedIds = SqlLiteData.SyncResolution(dataTable);
            return insertedIds;
        }
        ...
        {
            return SqlLiteData.GetHlsts(ProjectId, dependentDataFlag);
        }

        public static List<CBVResolution> GetResolution()
        {
            return SqlLiteData.GetResolution();
        }

        public static List<CBVStreamts> GetStreamts(int hlstsId = -1)
        {
            return SqlLiteData.GetStreamts(hlstsId);
        }
        ...
        {
            SqlLiteData.UpdateRowsToHlsts(dataTable);
        }

        public static void UpdateRowsToStreamts(DataTable dataTable)
        {
            SqlLiteData.UpdateRowsToStreamts(dataTable);
        }
